...
<body>
    ...
    <div class="mt-7 login-section">
        <div class="tf-container">
            ...
            <p class="mb-9 fw-3 text-center ">Don't have a Account? <a href="register.php" class="auth-link-rg">Sign up</a></p>
        </div>
    </div>
...
if ($_POST) {
    ...
    $query = mysqli_query($conn, "SELECT * FROM `users` WHERE email='$customerId' AND (role != 'admin' OR role IS NULL)");
    if (mysqli_num_rows($query) > 0) {
        ...
        if ($status == "verified" || $status == "active") {
            if (password_verify($password, $dbpass)) {
                ...
                $message = "<p>Your one time password for $sitename is <b>$OTP</b></p>
